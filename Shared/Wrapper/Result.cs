...
namespace Shared.Wrapper;
    ...
    public static string EERR {get; set; }
    ...
    public static IResult Fail(Failure failure)
    {
        return new Result { Succeeded = false, Failure = failure, Messages = new List<string> { failure.Message }, HttpCode = failure.StatusCode };
    }
    ...
    public static Result Faild(string message, int httpCode = 0)
    {
        return new Result { Succeeded = false, Messages = new List<string> { message }, HttpCode = httpCode };
    }    
    public static Result Faild(Failure failure)
    {
        return new Result { Succeeded = false,Failure= failure,  Messages = new List<string> { failure.Message }, HttpCode = failure.StatusCode };
    }
...
public class Result<T> : Result, IResult<T>
{
    ...
    public new static Result<T> Fail(Failure failure)
    {
        return new Result<T>
        {
            Succeeded = false,
            Failure = failure,
            Messages = new List<string> { failure.Message },
            HttpCode = failure.StatusCode
        };
    }
